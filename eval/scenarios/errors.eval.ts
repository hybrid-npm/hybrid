	...
	return [
		...
		},
		{
			...
			run: async (ctx: TestContext) => {
				...
				if (response.status >= 500) {
					throw new Error("Server error for missing chatId")
				}
			}
		},
		{
			...
			run: async (ctx: TestContext) => {
				...
				if (response.status !== 200) {
					throw new Error(
						`Empty message should be handled, got status ${response.status}`
					)
				}
			}
		},
		{
			...
			run: async (ctx: TestContext) => {
				...
				throw new Error(
					`Unexpected response for long message: ${response.status}`
				)
			}
		},
		{
			...
			run: async (ctx: TestContext) => {
				...
				for (let i = 0; i < 10; i++) {
					promises.push(
						ctx.http.post("/api/chat", {
							messages: [{ id: String(i), role: "user", content: "Ping" }],
				...
				if (successful === 0) {
					throw new Error(
						"All requests failed - possible rate limiting misconfiguration"
					)
				}
			}
		}
