...
export interface ScheduledTask<TPayload = unknown, TRunContext = unknown> {
	id: string
	name: string
...
export interface ScheduledTaskRun<TPayload = unknown, TRunContext = unknown> {
	taskId: string
	scheduledAt: Date
	startedAt: Date
	completedAt?: Date
...
export interface Scheduler<TPayload = unknown, TRunContext = unknown> {
	schedule(input: {
		name: string
		scheduleType: ScheduleType
		scheduleValue: string
	...
	}): Promise<ScheduledTask<TPayload, TRunContext>>
